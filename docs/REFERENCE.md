# RocketForge reference

Reference material that used to live in the root README, moved here unchanged
when the README became a product overview. Token values (colour, type,
spacing, motion) live in `ui/theme/`, and the component library in
`ui/components/`. Where this page and the source disagree, the source wins.
...
documented campaigns (`engineering/implementation/`,
...
| Thermochemistry (NASA CEA primary provider, Cantera dev-only independent oracle) | **Verified** — CEA and Cantera both confirmed correct against external published references, cross-provider consistency confirmed within an evidenced envelope; see `engineering/verification/CEA_CANTERA_VERIFICATION_R1.md` |
...
## Engine Design, as captured

![Engine Design: a seven-component gas-generator topology with the Main Chamber selected, its inspector open, and every quantitative readout shown as an em dash](images/screenshots/engine-design.png)

**Engine Design** — a seven-component LOX/CH₄ gas-generator topology with the
Main Chamber selected and its inspector open. This is the mode this page is
...
## Why PyInstaller, and a directory build
